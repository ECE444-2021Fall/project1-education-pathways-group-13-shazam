    ...
    for (let i = 0; i < sections.length; i++) {
      if (sections[i].id === event.target.id) {
        ...
        document.getElementById('details').innerHTML =
          '<h1>' + event.target.id + '</h1>' + "<br/><p>This is the user's " + event.target.id + '</p>';
      } else {
  ...
  const setSection = (event) => {
    ...
    for (let i = 1; i < sections.length; i++) {
      if (document.URL.indexOf('#' + sections[i].id) > -1) {
        ...
        document.getElementById('details').innerHTML =
          '<h1>' + sections[i].id + '</h1>' + "<br/><p>This is the user's " + sections[i].id + '</p>';
      } else {
        sections[i].classList.remove(styles.active_section);
      }
    }
      ...
      <main className={styles.main} onLoad={setSection}>
        <div className={styles.grid}>
          <div className={[styles.card, styles.leftbar].join(' ')}>
            ...
            <br />
            <Link href="">
              <div id="profile" className={styles.section} onClick={switchSection}>
                Profile
              </div>
            </Link>
            <Link href="#schedule">
              <div id="schedule" className={styles.section} onClick={switchSection}>
                Schedule
              </div>
            </Link>
            <Link href="#settings">
              <div id="settings" className={styles.section} onClick={switchSection}>
                Settings
              </div>
            </Link>
          </div>
